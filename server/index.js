...
app.post('/api/send-email', async (req, res) => {
  try {
    ...
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: 'Rino Enterprises <onboarding@resend.dev>', to: email, subject: 'Thank you for your purchase! We will be in contact within 24 hours.', html }),
    });

    await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { 'Authorization': 'Bearer ${process.env.RESEND_API_KEY}', 'Content-Type': 'application/json' },
      body: JSON.stringify({ from: 'Rino Enterprises <onboarding@resend.dev>', to: 'rinosocialenterprises@gmail.com', subject: 'New Purchase: ' + (planNames[tier] || 'Basic') + ' by ' + name, html: '<h2>New Purchase!</h2><p>Customer: ' + name + '</p><p>Email: ' + email + '</p>' }),
    });
...
app.post('/api/create-payment-intent', async (req, res) => {
  try {
    ...
    const paymentIntent = await stripe.paymentIntents.create({ amount: prices[tier] || prices.basic, currency: 'eur', automatic_payment_methods: { enabled: true }, receipt_email: email, metadata: { tier: tier || 'basic' } });
